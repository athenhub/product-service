 ...
 *   <li><b>트랜잭션 경계 분리</b>: 트랜잭션은 Register/Update/Delete 각 서비스 내부에서 관리되며,
 *       ApplicationService는 트랜잭션 오케스트레이션만 수행한다.
 *   <li><b>조회 분리</b>: ProductQueryService를 사용해 읽기 전용 조회를 수행하고, 이 결과를 기반으로 권한을 검증한다.
 * </ul>
 *
 * <p>ApplicationService는 도메인 로직을 직접 구현하지 않으며, 도메인 모델과 외부 시스템 간의 조정자(Coordinator)
 * 역할을 수행한다. 이는 DDD의 애플리케이션 계층 설계와 일치한다.
 */
@Service
...
public class ProductApplicationService {
     ...
     * 상품 등록 유스케이스를 실행한다.
     *
     * <p>등록 요청에 포함된 hubId, vendorId가 요청자의 권한과 일치하는지 PermissionPolicy로 검증한 후,
     * ProductRegisterService에 등록 작업을 위임한다.
     *
     ...
     *
     * <p>1) 상품 조회 (ProductQueryService)
     * 2) 권한 체크 (PermissionPolicy)
     * 3) 도메인 업데이트 실행 (ProductUpdateService)
     *
